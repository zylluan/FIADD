...
#设置 set -e和set -o nounset，意味着如果任何命令失败或使用了未定义的变量，脚本将停止执行。
# Fail on error 
set -e
...
# Get directory containing mininet folder 通过dirname和pwd -P命令确定Mininet目录的绝对路径
MININET_DIR="$( cd -P "$( dirname "${BASH_SOURCE[0]}" )/../.." && pwd -P )"

# Set up build directory, which by default is the working directory
#  unless the working directory is a subdirectory of mininet,
#  in which case we use the directory containing mininet
# 确定构建目录，默认为当前工作目录，如果当前目录是Mininet的子目录，则使用 Mininet 的父目录。
BUILD_DIR="$(pwd -P)"
case $BUILD_DIR in
...
# Attempt to identify Linux release 通过检查 /etc 目录下的版本文件来识别当前的 Linux 发行版和版本号。
...
# 检查环境变量PYTHON是否已经设置。如已设置，则使用该值；如未设置，默认使用python作为PYTHON变量的值
PYTHON=${PYTHON:-python}
#定义了一个字符串变量 PRINTVERSION，它包含了一个 Python 代码片段-代码作用:打印版本信息
PRINTVERSION='import sys; print(sys.version_info)'

#import sys;：这一行导入了 Python 的内置 sys 模块。sys 模块提供了访问 Python 解释器和它的环境的一些变量和函数，包括解释器的版本信息。
#print(sys.version_info)：这一行调用 sys 模块的 version_info 属性，并打印其值。sys.version_info 是一个元组，包含了 Python 版本的主要组成部分，通常包括主版本号、次版本号、微版本号等。

#$python -c "$PRINTVERSION":-c 选项表示执行随后的字符串作为 Python 代码,$PRINTVERSION 是一个变量，其值是 import sys; print(sys.version_info)将这些部分组合起来，这行代码的意思是：执行 $python -c "import sys; print(sys.version_info)" 来获取 Python 的版本信息。

...
/**
 * @file commander.cpp
 *~/PX4-Autopilot/build/px4_sitl_default/uORB/topics
...
/*定义飞行器模式标志，用于表示飞行器的不同工作模式typedef enum VEHICLE_MODE_FLAG {
    VEHICLE_MODE_FLAG_CUSTOM_MODE_ENABLED  = 1,   自定义模式启用 
    VEHICLE_MODE_FLAG_TEST_ENABLED         = 2,   测试模式启用 
    VEHICLE_MODE_FLAG_AUTO_ENABLED         = 4,   自动模式启用 
    VEHICLE_MODE_FLAG_GUIDED_ENABLED       = 8,   引导模式启用 
    VEHICLE_MODE_FLAG_STABILIZE_ENABLED    = 16,  稳定模式启用
    VEHICLE_MODE_FLAG_HIL_ENABLED          = 32,  硬件在环仿真模式启用 
    VEHICLE_MODE_FLAG_MANUAL_INPUT_ENABLED = 64,  手动输入启用 
    VEHICLE_MODE_FLAG_SAFETY_ARMED         = 128, 安全解锁 
    VEHICLE_MODE_FLAG_ENUM_END             = 129, 枚举结束 
} VEHICLE_MODE_FLAG;
*/
...
// TODO: generate
//重载了 == 运算符，用来比较两个 actuator_armed_s（飞控是否解锁的状态结构体）是否一致。
...
{
	vehicle_command_s vcmd{}; //是 PX4 定义的 uORB 消息类型，用于存储 飞行器指令
	vcmd.command = cmd;
	vcmd.param1 = param1;
	vcmd.param2 = param2;
	vcmd.param3 = param3;
	...
	//uORB::SubscriptionData：PX4 的 uORB 订阅器，用于读取 vehicle_status_s 消息。
	//ORB_ID(vehicle_status)：表示订阅 飞行器状态 主题。
	...
	//uORB::Publication：PX4 的 uORB 发布器，用于向 vehicle_command 主题发布消息。
	uORB::Publication<vehicle_command_s> vcmd_pub{ORB_ID(vehicle_command)};
	//hrt_absolute_time() 获取当前 系统时间戳（微秒级），用于同步指令。
	vcmd.timestamp = hrt_absolute_time();
	//将 vcmd 发送到 vehicle_command 主题，使得 PX4 其他模块可以接收到该指令
	return vcmd_pub.publish(vcmd);
}

//在 100ms 内监听 vehicle_command_ack 回复，用于判断命令是否被接受。
...
//函数开头检查 Commander 是否运行中，如果没有运行，则打印帮助并返回错误。
...
//条件编译：非受限闪存下才允许这些命令：
	...
	//命令：arm 和 disarm 上锁与解锁
	if (!strcmp(argv[0], "arm")) {
		float param2 = 0.f;

		// 21196: force arming/disarming (e.g. allow arming to override preflight checks and disarming in flight) 参数 -f 可强制跳过 preflight checks
	...
	if (!strcmp(argv[0], "disarm")) {
		...
		//argc > 1：表示命令行至少包含两个参数;strcmp(a, b) 比较字符串是否相同，相同返回 0
		if (argc > 1 && !strcmp(argv[1], "-f")) {
			param2 = 21196.f;
		}
	...
	if (!strcmp(argv[0], "takeoff")) {
		...
		//先发送 VEHICLE_CMD_NAV_TAKEOFF 起飞命令，等待 ACK 后再发送 arm 指令。
	...
	//命令：set_ekf_origin 设置 EKF 起始定位点 需要三个参数：纬度 经度 海拔
...
{
	/* only handle commands that are meant to be handled by this system and component, or broadcast 只处理本系统和组件应该处理的命令，或者广播*/
	...
	case vehicle_command_s::VEHICLE_CMD_DO_REPOSITION: {
			...
			// the data at the exact same time.在这里切换飞行模式，导航器会处理一些合理的坐标。它的设计不需要导航器和命令来同时接收/处理数据。
	...
	case vehicle_command_s::VEHICLE_CMD_NAV_VTOL_TAKEOFF:
		...
		/* ok, home set, use it to take off */
		//_user_mode_intention.change(...) 试图切换导航模式
...
void Commander::run()
{
	/* initialize 初始化 LED 和蜂鸣器。*/
	led_init();
	buzzer_init();

/*
如果支持电源按钮：
发布 power_button_state 和 tune_control，用于响应按钮事件或播放提示音；
注册电源状态通知的回调函数。
*/
#if defined(BOARD_HAS_POWER_CONTROL)
	{
	...
	_boot_timestamp = hrt_absolute_time(); //设置开机时间戳： 用于后续判断是否刚开机、超时等逻辑。

	arm_auth_init(&_mavlink_log_pub, &_vehicle_status.system_id);//初始化飞行授权验证：初始化 Arm 授权模块，用于限制非法起飞。
	...
	while (!should_exit()) {
		...
		/* update parameters 🎯 更新参数（比如遥控器配置、系统ID等）：*/
...
*/
		...
		//如果初始化完成，且未标记为校准状态，自动进入 STANDBY 状态（待命）
		...
		//🗺️ 检查任务 & 控制模式：
		checkForMissionUpdate(); //检查任务更新；
		manualControlCheck(); //检查是否进入手动控制；
		offboardControlCheck(); //检查是否进入 offboard 模式；
		// data link checks which update the status 数据链检查（是否与地面站连通）：
		dataLinkCheck();

		// Check for failure detector status故障检测器更新：如果有新的故障状态，记录下来。
		...
		// Run arming checks @ 10Hz  10Hz飞行前健康检查：
...
*/
		...
		_actuator_armed.prearmed = getPrearmState();  //Prearm 逻辑：
//📤 发布核心状态（armed 状态、模式、vehicle_status、failure 状态等）：
//如果状态发生变化，立即发布；否则默认 2Hz 发布一次。
		...
		checkWorkerThread(); // 子线程检查
		updateTunes();
		control_status_leds(_status_changed, _battery_warning); //LED 状态控制
		//关闭循环状态标志 & 更新状态
		_status_changed = false;
		_was_armed = _arm_state_machine.isArmed();
		//飞行授权持续验证：
		arm_auth_update(hrt_absolute_time(), params_updated);
		//若处于锁定状态，则开启 reset lockout：
...
/*
这段函数 Commander::landDetectorUpdate() 是 PX4 Commander 中用于**更新“着陆状态”**的关键逻辑。它会根据 land_detector 模块发布的消息，判断是否发生了起飞或降落事件，并据此触发相应操作，比如设置 home 点、记录起飞时间等。*/
void Commander::landDetectorUpdate()
{
//如果有新的着陆检测消息更新（订阅的 vehicle_land_detected 主题），进入逻辑处理。
	if (_vehicle_land_detected_sub.updated()) {
		//保存更新前的“是否着陆”状态，便于后面对比新旧状态判断是否有“起飞/降落”事件发生。
		const bool was_landed = _vehicle_land_detected.landed;
		//从 land_detector 订阅器中拷贝最新的状态到 _vehicle_land_detected，更新当前着陆状态。
		_vehicle_land_detected_sub.copy(&_vehicle_land_detected);

		// Only take actions if armed仅在飞行器处于解锁（armed）状态下才处理着陆检测结果，否则可能是地面调试时误触发。
		if (_arm_state_machine.isArmed()) {
			//如果原本没着陆，现在检测到着陆，说明是“降落完成”。
			if (!was_landed && _vehicle_land_detected.landed) {
				//通过 MAVLink 记录日志，并广播一个事件：commander_landing_detected
			...
			//如果原本处于着陆状态，现在变为“未着陆”，说明是刚刚起飞。
				//同样记录日志和事件：commander_takeoff_detected。
				...
				//记录起飞时间（用于后续比如 failsafe 判断飞行时间等）
				_vehicle_status.takeoff_time = hrt_absolute_time();
				//设置“自解锁以来是否起飞过”的标志位。
				_have_taken_off_since_arming = true;
			}
			//如果参数 COM_HOME_EN 允许自动设置 home 点，则进行如下逻辑：
			...
			if (_param_com_home_en.get()) {
				...
				//在“起飞”瞬间设置地面 home 点,当前是飞行中（即将起飞）、且从开机已过去 500ms 以上（防止空中重启后误设置），并且是从“着陆”状态刚起飞的，此时设置“地面 home 点”。
				if (!_vehicle_land_detected.landed && (hrt_elapsed_time(&_boot_timestamp) > INAIR_RESTART_HOLDOFF_INTERVAL)) 
				{
					...
					//情况B：如果参数允许，可在空中设置 home 点,如果未处于“从地面起飞”的情形，但允许空中设置 home 点（COM_HOME_IN_AIR 为真），则在飞行中设置“空中 home 点”。
...
//这是一个仅在 SITL 模拟器中编译的函数，主要功能是：在终端提示是否满足起飞条件。
void Commander::checkAndInformReadyForTakeoff()
{
#ifdef CONFIG_ARCH_BOARD_PX4_SITL//这是一个仅在 SITL 模拟器中编译的函数，主要功能是：在终端提示是否满足起飞条件。
	static bool ready_for_takeoff_printed = false; //静态变量，确保只打印一次“Ready for takeoff”提示。
	//仅当飞行器类型为“旋翼机”或“固定翼”时，才执行以下逻辑（避免如 VTOL 特殊模式下误提示）。
...
//这是一个核心函数，用于更新并发布当前飞行控制模式（vehicle_control_mode）。
void Commander::updateControlMode()
{
	//清空当前 _vehicle_control_mode，初始化为默认空结构体。
	_vehicle_control_mode = {};
	/*
	调用 mode_util 工具函数，根据以下参数生成控制模式：
当前是否已解锁；
当前导航状态；
当前飞行器类型（固定翼/多旋翼/VTO）；
当前接收到的 offboard 控制模式（通过订阅）；
输出结果是填充到 _vehicle_control_mode 中。*/
	//getVehicleControlMode() 是一套根据状态逻辑组合来设定飞控控制权限（位置控制、姿态控制、offboard 是否允许等）的函数。
	...
	_vehicle_control_mode.timestamp = hrt_absolute_time();//给 vehicle_control_mode 添加当前时间戳，用于追踪消息的时间同步。
	_vehicle_control_mode_pub.publish(_vehicle_control_mode);//发布 vehicle_control_mode 消息，这个消息是整个 PX4 控制系统用于决策的重要依据，例如控制器、offboard模块等都依赖它判断当前控制权限是否允许。
}
...
int Commander::print_usage(const char *reason)
{
	...
	PRINT_MODULE_DESCRIPTION(
		//R"DESCR_STR(...)" 是 C++ 的原始字符串 语法，可以直接包含换行符，避免使用 \n
		R"DESCR_STR(
### Description
The commander module contains the state machine for mode switching and failsafe behavior.
)DESCR_STR");
	//设定该模块的名称为 "commander"，类别为 "system"
	PRINT_MODULE_USAGE_NAME("commander", "system");
	//表示 commander 模块可以用 commander start 命令启动
	PRINT_MODULE_USAGE_COMMAND("start");
	//-h 参数用于启用 HIL（Hardware-in-the-Loop）模式
	PRINT_MODULE_USAGE_PARAM_FLAG('h', "Enable HIL mode", true);
//使用 #ifndef CONSTRAINED_FLASH 来判断是否启用了 受限存储模式
#ifndef CONSTRAINED_FLASH
	//calibrate 用于运行传感器校准。
	PRINT_MODULE_USAGE_COMMAND_DESCR("calibrate", "Run sensor calibration");
	//可以选择校准类型，如 mag（磁力计）、baro（气压计）、accel（加速度计）等。
	//quick 选项允许快速校准（仅加速度计），但不推荐使用
	...
	//check 用于运行预飞检查，确保飞行器在起飞前处于安全状态。
	PRINT_MODULE_USAGE_COMMAND_DESCR("check", "Run preflight checks");
	//arm 用于解锁飞行器的电机和控制系统，使其准备起飞。
	PRINT_MODULE_USAGE_COMMAND("arm");
	//-f 参数用于强制解锁，即使预飞检查未通过。
	PRINT_MODULE_USAGE_PARAM_FLAG('f', "Force arming (do not run preflight checks)", true);
	//disarm 用于锁定飞行器的电机和控制系统，使其处于安全状态。
	//-f 参数用于强制解锁，即使飞行器在空中。
	...
	//transition 是vtol（垂直起降）模式的转换命令。
	PRINT_MODULE_USAGE_COMMAND_DESCR("transition", "VTOL transition");
	//mode 用于更改飞行模式。
	//可以选择手动模式、空中模式、离线模式、稳定模式、高度控制模式、位置控制模式、自动任务模式 自动盘旋 自动返航 自动起飞/ 降落 精准降落等。
...
#endif
	//PRINT_MODULE_USAGE_DEFAULT_COMMANDS Prints the default commands: stop & status.
